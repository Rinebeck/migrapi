...
try {
    ...
    if (isset($postdata)) {
        ...
        if(isset($request->city) && isset($request->datas) ){
            ...
            $insert = "INSERT INTO `pollution_city` (`air_quality`, `water_drinking`, `garbage`, `clean`,
                               `noise_light`, `water_pollution`, `comfortable`, `parks`, `city_id`) VALUES (";
            ...
            $insert.=", ".$cityId[0].")";
            $pollution = $conexion->query($insert);
        ...
        //$pollution = [0 => $result, 1 => $e->getMessage()];
    echo json_encode([ false, '', '¡ERROR! Faltan datos, intente de nuevo.']);
    };
    exit();
?>